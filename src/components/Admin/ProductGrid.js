    ...
    setFilters({ ...filters, [name]: value });
     ...
     useEffect(() => {
        let filtered = products.filter(product => {
            const productPrice = product.productVariants?.[0]?.pricePerUnit;
            return (
    ...
    return (
       ...
       {productData.productVariants.map((variant, index) => (
        <div key={index} className="variant-row">
            ...
            <button type="button" style={{backgroundColor:'#dc3545'}} onClick={() => handleRemoveVariant(index)}>Remove</button>
        )}
        </div>
        
    ))}
     <button type="button" style={{backgroundColor:'#007bff'}} onClick={handleAddVariant}>Add Variant</button>
