...
const ProfilePage = () => {
  ...
  const fetchUserProfile = async () => {
    ...
    const email = decodedToken.email && Array.isArray(decodedToken.email) ? decodedToken.email[0] : decodedToken.email;  
...
const handleCancelOrder = async (orderId, shipping) => {
  setCancelingOrders((prevCancelingOrders) => ({ ...prevCancelingOrders, [orderId]: true }));
      try {
        ...
        if (currentDate > shippingDate) {
          if (currentDate < estimatedDeliveryDate) {
            showErrorToast("You cannot cancel the order as it is already shipped.");
          } else {
            showErrorToast("You cannot cancel the order as it is already delivered.");
          }
        ...
        if (currentOrder.status === "Cancelled") {
         showInfoToast("This order has already been canceled.");
        ...
        setCancelingOrders((prevCancelingOrders) => ({ ...prevCancelingOrders, [orderId]: false }));
      }
    };
...
<button
  ...
  <div className="profile-order-details">
      ...
      <div>
       {order.status === 'Cancelled' || cancelledOrders.has(order.id)? (
      ...
      disabled={cancelingOrders[order.id]}
    >
      {cancelingOrders[order.id] ? (
        <span>
          Canceling... <i className="fas fa-spinner fa-spin"></i>
        </span>
      ) : (
        "Cancel Order"
      )}
