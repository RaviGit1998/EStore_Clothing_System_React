
// import React, { useContext, useEffect, useState } from 'react';
// import { useParams,useOutletContext } from 'react-router-dom';
// import './ProductDetails.css';
// import { CartContext } from '../Cart/CartContext';
// import { showErrorToast, showInfoToast, showSuccessToast, showWarningToast } from '../Toasting/ThrottledToast';
// import 'react-toastify/dist/ReactToastify.css';
// import { useNavigate } from 'react-router-dom';
// import placeOrder from '../PlaceOrder/PlaceOrder';
// const ProductDetails = () => {
//     const navigate=useNavigate();
//     const { addToCart } = useContext(CartContext);
//     const { id } = useParams();
//     const [product, setProduct] = useState(null);
//     const [loading, setLoading] = useState(true);
//     const [error, setError] = useState(null);
//     const [isInWishlist, setIsInWishlist] = useState(false);    
//     useEffect(() => {
//         fetch(`https://localhost:7181/api/Product/${id}`)
//             .then(response => {
//                 if (!response.ok) {
//                     throw new Error('Network response was not ok');
//                 }
//                 return response.json();
//             })
//             .then(data => {
//                 setProduct(data);
//                 setLoading(false);
//             })
//             .catch(err => {
//                 setError(err);
//                 setLoading(false);
//             });
//     }, [id]);

//     const handleAddToCart = () => {
//         const token = localStorage.getItem('jwtToken');
       
//         if (!token) {
//            showWarningToast("Please login to order");
//             navigate('/login');
//             return;
//         }
//         const selectedVariant = product.productVariants?.[0];

//         // Check if the selected variant is available
//         if (!selectedVariant || selectedVariant.quantity <= 0) {
//             showErrorToast("Product is out of stock"); // Show toast if out of stock
//             return; // Exit the function if the product is out of stock
//         }

//   let cartContainer = JSON.parse(localStorage.getItem('cartContainer')) || [];
//     const existingProduct = cartContainer.find(item => item.productId === product.productId);
//     if (existingProduct) {
//         // If the product is already in the cart, show a notification
//         showInfoToast("Product is already in the cart");
//     } else {
//         // If not in the cart, add the product
//         cartContainer.push(product);
//         localStorage.setItem('cartContainer', JSON.stringify(cartContainer));

//         // Add to cart context (if you're using context for cart management)
//         addToCart(product);

//         // Show success notification
//         showSuccessToast("Product added to the Cart");
//       }
//     };

//     const handleBuyNow =async () => {
//         // const Id = await placeOrder([product]);
//         // navigate(`/order-summary/${Id}`, { state: { orderItems: [product] } });
//         const token = localStorage.getItem('jwtToken');
       
//         if (!token) {
//             showWarningToast("Please login to order");
//             navigate('/login');
//             return;
//         }
//         const selectedVariant = product.productVariants?.[0];
 
//         if (!selectedVariant || selectedVariant.quantity <= 0) {
//             showErrorToast("Product is out of stock"); // Show toast if out of stock
//             return; // Exit the function if the product is out of stock
//         }
//            const quantity = 1; // default quantity, you can adjust this as needed
//          const productWithVariants = { ...product, quantity };
//         const Id = await placeOrder([product]);
//         navigate(`/order-summary/${Id}`, { state: { orderItems: [productWithVariants] } });
//     };
    
//     const handleAddToWishlist = () => {
//         setIsInWishlist(!isInWishlist);
//         let wishlist = JSON.parse(localStorage.getItem('wishlist')) || [];
//         if (!wishlist.find(item => item.productId === product.productId)) {
//             wishlist.push(product);
//             localStorage.setItem('wishlist', JSON.stringify(wishlist));
//          showSuccessToast("item added to wishlist")
//         } else {
//            showWarningToast("item is already in wishlist");
//         }
//     };
//     if (loading) return <p className="loading">Loading...</p>;
//     if (error) return <p className="error">Error: {error.message}</p>;
//     if (!product) return <p className="not-found">Product not found.</p>;

//     return (
//         <div className="product-details">
//             <div className="product-image-section">
//                 <div className="product-image-list">
//                     {product.imageBase64 ? (
//                         <img
//                             src={`data:image/png;base64,${product.imageBase64}`}
//                             alt={product.name}
//                             className="product-image-main"
//                             style={{height:'500px'}}
//                         />
//                     ) : (
//                         <img
//                             src="fallback-image.jpg"
//                             alt={product.name}
//                             className="product-image-main"
//                         />
//                     )}
//                 </div>
//                 <div className="product-actions">
//                 <span className="icon-style2" onClick={handleAddToWishlist}>
//                    {isInWishlist ? (
//                     <i className="fa-solid fa-heart"></i>
//                 ) : (
//                     <i className="fa-regular fa-heart"></i>
//                 )}
                 
//                     </span>
//                     <button className="btn7 btn btn-add-to-cart" onClick={handleAddToCart}>
//                         Add to Cart
//                     </button>                 
//                     <button className="btn7 btn btn-buy-now" onClick={handleBuyNow}>
//                         Buy Now
//                     </button>
//                 </div>
//             </div>

//             <div className="product-info">
//                 <h1 className="product-title">{product.name}</h1>
//                 <p className="product-brand"><strong>Brand:</strong> {product.brand}</p>
//                 <p className="product-price"><strong>Price:</strong> ₹{product.productVariants?.[0]?.pricePerUnit || '0'}</p>

//                 {product.shortDescription && (
//                     <p className="product-description"><strong>Description:</strong> {product.shortDescription}</p>
//                 )}
//                 {product.longDesrciption && (
//                     <p className="product-details-description"><strong>Details:</strong> {product.longDesrciption}</p>
//                 )}
// {/* 
//                 <p className="product-category"><strong>Category:</strong> {product.categoryId}</p>
//                 <p className="product-subcategory"><strong>Subcategory:</strong> {product.subCategoryId}</p> */}

//                 <h3>Variants:</h3>
//                 {product.productVariants && product.productVariants.length > 0 ? (
//                     <ul className="product-variants">
//                         {product.productVariants.map(variant => (
//                             <li key={variant.productVariantId}>
//                                 <p><strong>Variant Name:</strong> {variant.name}</p>
//                                 <p><strong>Size:</strong> {variant.size}</p>
//                                 <p><strong>Color:</strong> {variant.color}</p>
//                                 <p><strong>Price Per Unit:</strong> ₹{variant.pricePerUnit || '0'}</p>
//                                 <p><strong>Quantity Available:</strong> {variant.quantity > 0 ? variant.quantity : "Out of Stock"}</p>
//                             </li>
//                         ))}
//                     </ul>
//                 ) : (
//                     <p>No variants available</p>
//                 )}

//                 <p className="product-created-date"><strong>Created on:</strong> {new Date(product.createdDate).toLocaleDateString()}</p>
//                 <p className="product-modified-date"><strong>Last Modified on:</strong> {new Date(product.modifiedDate).toLocaleDateString()}</p>
//             </div>
           
//         </div>
//     );
// };

// export default ProductDetails;

import React, { useContext, useEffect, useState } from 'react';
import { useParams } from 'react-router-dom';
import './ProductDetails.css';
import { CartContext } from '../Cart/CartContext';
import { showErrorToast, showInfoToast, showSuccessToast, showWarningToast } from '../Toasting/ThrottledToast';
import 'react-toastify/dist/ReactToastify.css';
import { jwtDecode } from 'jwt-decode';
import { useNavigate } from 'react-router-dom';
import placeOrder from '../PlaceOrder/PlaceOrder';
import { useDispatch } from 'react-redux';
import { addItem } from '../Reducers/cartSlice';

const ProductDetails = () => {
    const navigate=useNavigate();
    const { addToCart } = useContext(CartContext);
    const { id } = useParams();
    const [product, setProduct] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [selectedVariant,setSelectedVariant]=useState();
    const [wishlist, setWishlist] = useState([]);
    const [isInWishlist, setIsInWishlist] = useState(false);  
    const [selectedQuantity, setSelectedQuantity] = useState(1); // Default to 1  
    useEffect(() => {
        fetch(`https://localhost:7181/api/Product/${id}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
                return response.json();
            })
            .then(data => {
                setProduct(data);
                setLoading(false);
            })
            .catch(err => {
                setError(err);
                setLoading(false);
            });
    }, [id]);
 
    useEffect(() => {
        const token = localStorage.getItem('jwtToken');
        const userId = localStorage.getItem('userId');
 
        if (token) {
            fetch(`https://localhost:7181/api/Wishlist/${userId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
            .then(response => response.json())
            .then(data => {
               
                setWishlist(data);
                setIsInWishlist(data.some(item => item.productId === parseInt(id)));
            })
            .catch(err => {
                console.error('Error fetching wishlist:', err);
            });
        }
    }, [id]);
    useEffect(() => {
        if (product && product.productVariants.length > 0) {
            const defaultVariant = product.productVariants[0];
            setSelectedVariant(defaultVariant);
        }
    }, [product]);
    
    const handleVariantChange = (event) => {
        const selectedSize = event.target.value;
        const selected = product.productVariants.find(variant => variant.size === selectedSize);
        console.log("Selected Variant:", selected);
        setSelectedVariant(selected);
    };
    
    
    const handleQuantityChange = (event) => {
        setSelectedQuantity(Number(event.target.value));
    };

    const handleAddToCart = () => {
        console.log("Selected Variant on Add to Cart:", selectedVariant);
        
        if (!product) {
          console.error("Product is null or undefined");
          return;
        }
        if (!selectedVariant || selectedVariant.quantity <= 0) {
            showErrorToast("Product is out of stock"); // Show toast if out of stock
            return; // Exit the function if the product is out of stock
        }
        let cartContainer = JSON.parse(localStorage.getItem('cartContainer')) || [];
        const existingProduct = cartContainer.find(item => item.selectedVariant.productVariantId === selectedVariant.productVariantId);
        if (existingProduct) {
            // If the product is already in the cart, show a notification
            showInfoToast("Product is already in the cart");
        } else {
            cartContainer.push({ ...product, selectedVariant });
        localStorage.setItem('cartContainer', JSON.stringify(cartContainer));
        addToCart({ ...product, selectedVariant });
        showSuccessToast("Product added to the Cart");
         }
      };

    //with redux 

// const handleAddToCart = () => {
//     const dispatch = useDispatch();
//     console.log("Selected Variant on Add to Cart:", selectedVariant);
    
//     if (!product) {
//         console.error("Product is null or undefined");
//         return;
//     }
//     if (!selectedVariant || selectedVariant.quantity <= 0) {
//         showErrorToast("Product is out of stock"); // Show toast if out of stock
//         return; // Exit the function if the product is out of stock
//     }

//     // Dispatch the action to add item to Redux store
//     dispatch(addItem({ ...product, selectedVariant }));

//     showSuccessToast("Product added to the Cart");
// };

    const handleBuyNow = async () => {

        if (!selectedVariant || selectedVariant.quantity <= 0) {
           showErrorToast("Product is out of stock");
            return;
        }
        try {
            const quantity = selectedQuantity;
            const Id = await placeOrder([{ ...product,quantity, selectedVariant }]);
            navigate(`/order-summary/${Id}`, { state: { orderItems: [{ ...product,quantity, selectedVariant }] } });
        } catch (error) {
           showErrorToast("Failed to place the order");
            console.error(error);
        }
    };

    const handleAddToWishlist = () => {
        const token = localStorage.getItem('jwtToken');
        
        const decodedToken = jwtDecode(token);
       
        const userId = decodedToken.sub;  
       
        if (!token) {
            showWarningToast("Please login to add to wishlist");
            navigate('/login');
            return;
        }
   
        const url = `https://localhost:7181/api/Wishlist/${isInWishlist ? 'remove' : 'add'}`;
        const method = isInWishlist ? 'DELETE' : 'POST';
   
        fetch(url, {
            method,
            headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                userId: userId,
                productId: product.productId
            })
        })
        .then(response => {
            if (response.ok) {
                setIsInWishlist(!isInWishlist); // Toggle wishlist state
                showSuccessToast(isInWishlist ? "Item removed from wishlist" : "Item added to wishlist");
            } else {
                showErrorToast("Failed to update wishlist");
            }
        })
        .catch(err => {
            console.error('Error updating wishlist:', err);
            showErrorToast("Failed to update wishlist");
        });
    };


    if (loading) return <p className="loading">Loading...</p>;
    if (error) return <p className="error">Error: {error.message}</p>;
    if (!product) return <p className="not-found">Product not found.</p>;
 
    return (
        <div className="product-details">
        <div className="product-image-section">
            <div className="product-image">
                {product.imageBase64 ? (
                    <img
                        src={`data:image/png;base64,${product.imageBase64}`}

                        alt={product.name}
                        className="product-image-main"
                        style={{height:'auto',width:'370px',marginLeft:'20px'}}
                    />
                ) : (
                    <img
                        src="fallback-image.jpg"
                        alt={product.name}
                        className="product-image-main"
                        
                    />
                )}
            </div>
            <div className="product-actions">
                <span className="icon-style2" onClick={handleAddToWishlist}>
                    <i className="fa-solid fa-heart"></i>
                </span>
                <button className="btn7 btn btn-add-to-cart" onClick={handleAddToCart}>
                    Add to Cart
                </button>                 
                <button className="btn7 btn btn-buy-now" onClick={handleBuyNow}>
                    Buy Now
                </button>
            </div>
        </div>

        <div className="product-info">
            <h1 className="product-title">{product.name}</h1>
            <p className="product-brand"><strong>Brand:</strong> {product.brand}</p>
            
         

                {/* Price and Quantity based on selected size */}
         
            <p><strong>Color : </strong>{selectedVariant?.color|| ''}</p>
            {product.shortDescription && (
                <p className="product-description"><strong>Description:</strong> {product.shortDescription}</p>
            )}
            {product.longDesrciption && (
                <p className="product-details-description"><strong>Details:</strong> {product.longDesrciption}</p>
            )}
            <p> <strong>Size :</strong>  <select onChange={handleVariantChange} style={{width:'60px'}}>
           
           {product?.productVariants?.length > 0 ? ( 
           product.productVariants.map(variant => (
             <option key={variant.productVariantId} value={variant.size}>
              {variant.size}
             </option>
           ))
           ) : (
           <option disabled>No variants available</option>
           )}
           
           </select></p>
           
                       
           <p className="product-price"><strong>Price:</strong> ₹{selectedVariant?.pricePerUnit || '0'}</p>
                      {/* Quantity dropdown for available quantities or out of stock message */}
                      <p className="product-quantity">
                               <strong>Quantity:</strong>
                               
                               {selectedVariant?.quantity > 0 ? (
                                  <select value={selectedQuantity} onChange={handleQuantityChange} style={{ width: '60px', marginLeft: '10px' }}>
                                  {/* Determine the maximum quantity to display: the lesser of available stock or 5 */}
                                  {[...Array(Math.min(selectedVariant.quantity, 5)).keys()].map(num => (
                                      <option key={num + 1} value={num + 1}>{num + 1}</option>
                                  ))}
                              </select>
                               ) : (
                                   <span style={{ color: 'red', marginLeft: '10px' }}>Out of Stock</span>
                               )}
                           </p>
           

            <p className="product-created-date"><strong>Created on:</strong> {new Date(product.createdDate).toLocaleDateString()}</p>
            <p className="product-modified-date"><strong>Last Modified on:</strong> {new Date(product.modifiedDate).toLocaleDateString()}</p>
        </div>
      
    </div>
    );
};
 
export default ProductDetails